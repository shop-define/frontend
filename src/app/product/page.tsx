...
export default function ProductCardPage(): JSX.Element {
  const getImageItem = (): JSX.Element[] => {
    const images = []
    for (let i = 0; i < 5; i++) {
      images.push(
        <Image
      ...
      <Flex className='root'>
        <Breadcrumb
          items={[
            ...
            {
              title: <a href=''>Одежда</a>,
            },
            {
              title: <a href=''>Футболки</a>,
            },
            {
              title: <a href=''>Джерси</a>,
            },
          ]}
        />
        <Flex vertical gap={10}>
          <Image width={'45%'} className='product-image' placeholder style={{ aspectRatio: '1' }} />
          <Flex gap={10}>{getImageItem()}</Flex>
        </Flex>
      </Flex>
      <Footer />
    </>
